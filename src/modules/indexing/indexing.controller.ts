...
export class IndexingController {
    ...
    async handlePostCreated(
        ...
        try {
            this.logger.log(`[post.created] Received event for postId: ${data.postId}`);
            ...
            this.logger.log(`[post.created] Successfully indexed postId: ${data.postId}`);
        } catch (error) {
            this.logger.error(`[post.created] Failed to index postId: ${data?.postId}`, error);
